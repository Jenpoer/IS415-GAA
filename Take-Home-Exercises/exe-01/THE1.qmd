...
```{r}
wp_sf_osun <- wp_sf %>% dplyr::select(c("Geometry", "status_clean", "#clean_country_name", "#clean_adm1", "#clean_adm2"))
glimpse(wp_sf_osun)
```
...
tm_basemap("OpenStreetMap") +
  ...
  tm_shape(osun) +
    tm_borders(lwd=0.5) +
  tm_shape(st_intersection(wp_sf_osun, osun)) +
...
func_points <- tm_shape(osun) +
  tm_fill() +
  tm_shape(st_intersection(functional, osun)) +
  tm_dots() +
...
non_func_points <- tm_shape(osun) +
  tm_fill() +
  tm_shape(st_intersection(non_functional, osun)) +
  tm_dots() +
...
We can observe from the plot that the computed G-function values lie above the envelope, indicating some clustering. Therefore, we can reject the null hypothesis that functional water points in Ife East are randomly distributed at 95% confidence interval.
...
Using the functional and non_functional dataframes that we have created during the Data Preprocessing section, we will extract the functional and non-functional points into vectors.

```{r}
functional_list <- functional$func_status

non_functional_list <- non_functional$func_status
```
...
```{r}
nb <- include_self(st_knn(st_geometry(wp_sf_osun), 6))
```
...
```{r}
wt <- st_kernel_weights(nb, wp_sf_osun, "gaussian", adaptive=TRUE)
```
